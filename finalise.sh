...
if [ -f "$ACTIVE_FILE_PATH" ] ; then
    ACTIVE_LABEL=$(cat "$ACTIVE_FILE_PATH" | head -1 )
    ACTIVE_PATH=$(cat "$ACTIVE_FILE_PATH" | head -2 )
    if [ -d "$ACTIVE_PATH" ] ; then
        ...
        if [ $NUM_VIDEO_FILES -gt 1 ] ; then
            ...
            RENAME_FORMAT="$OUTPUT_PATH"/"$VIDEO_TELEVISION_DIR"/"$VIDEO_TELEVISION_FORMAT"
        else
            # Movie
            QUERY_DB="$VIDEO_MOVIES_DB"
            RENAME_FORMAT="$OUTPUT_PATH"/"$VIDEO_MOVIES_DIR"/"$VIDEO_MOVIES_FORMAT"
        fi
