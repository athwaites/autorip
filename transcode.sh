...
while true; do
    ...
    for CUR_IN_PATH in $(ls -tr "$WORKING_PATH"/*/*.mkv) ; do
        ...
        "$TRANSCODER_BIN" -i "$CUR_IN_PATH" -c:v "$TRANSCODER_VIDEO_FORMAT" -preset "$TRANSCODER_VIDEO_PRESET" -crf "$TRANSCODER_VIDEO_CRF" -max_muxing_queue_size 9999 -c:a "$TRANSCODER_AUDIO_FORMAT" -y "$CUR_OUT_PATH"
